package io.bega.phonetutorial.activities;


import io.bega.phonetutorial.R;
import io.bega.phonetutorial.utils.Utils;
import io.bega.phonetutorial.views.ButtonFloat;
...
public class TutorialActivity extends Activity implements OnTouchListener, OnClickListener{
	...
	private void addImages(){
		for(int resource : images){
			...
			Bitmap resized = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(getResources(), resource, options), width, height, true);
	...
	protected void onDestroy() {
		if (imageContainer != null)
		{
			...
			int count = imageContainer.getChildCount();
			View v = null;
			for(int i=0; i<count; i++) {
				v = imageContainer.getChildAt(i);
