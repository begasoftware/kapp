 ...
 * STOP
 * Letter “U”
 * Action: Stop the measurements.
 * Final character(ACK): “+”
 * App action: Just notify the action.
 *
 */
public class StopContinuousDataReadAction extends BluetoothAction {
    Bus bus;

    public StopContinuousDataReadAction(Activity activity,
    ...
    protected void onPostExecute(Void aVoid) {
        if (datareceived!=null && datareceived.indexOf("+")!=-1){
            ...
            statusData.StatusKduino = 0;

            bus.post(statusData);
            DisplayUtilities.ShowLargeMessage(this.activity.getString(R.string.bluetooth_data_stop_kduino),
                    "",
        ...
        else
        {
            DisplayUtilities.ShowLargeMessage(
                            this.activity.getString(R.string.bluetooth_data_cant_stop_kduino),
                    "",
                    this.activity.findViewById(android.R.id.content),
                    false,
                    null);
    ...
    protected void onPreExecute() {
        try
        {
            this.bluetoothService.sendMessage("U");
        }
